...
//#endif
	...
	g_core_log->debug("Starting Libevent %s", event_get_version());
	g_core_log->debug("event base method %s", *event_get_supported_methods());
}
...
{
	g_core_log->error("libevent error %s", msg);
}

void ReactorCore::on_event_fatal(int error)
{
	g_core_log->error("event fatal error code %d", error);
}
