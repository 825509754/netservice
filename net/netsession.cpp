...
void net_session::on_event(short event)
{
	...
	if (error != 0)
	{
		g_core_log->error("socket error is %s", evutil_socket_error_to_string(error));
	}

	else if (event & BEV_EVENT_TIMEOUT)
	{
		g_core_log->error("bufferevent Timed out");
	}
	else if (event & BEV_EVENT_EOF)
	{
		g_core_log->debug("connection closed");
	}
	else if (event & BEV_EVENT_ERROR)
	{
		g_core_log->error("some other error");
	}
